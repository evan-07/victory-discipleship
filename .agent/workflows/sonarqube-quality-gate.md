...
- **Coverage < 80%:** The project uses TestSprite as the AI automated test generator. Use `testsprite_generate_code_and_execute` to automatically repair low code coverage on untested files. Make sure frontend tests output `lcov.info`.
- Use `testsprite_open_test_result_dashboard` for visually inspecting failing tests generated by TestSprite.
