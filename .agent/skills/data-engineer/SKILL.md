...
3.  **Gold Layer (Aggregation):**
    ...
    * **Dataform-to-Looker Coupling Protocol:** If `impact_analysis.sh` flags a potential Looker breakage, STOP. You must generate a **"Looker Impact Statement"** artifact detailing the specific schema alterations (fields added/removed/renamed), affected dashboards/reports, and whether the change is breaking or non-breaking. Ping the `@orchestrator` to assign this artifact to the `@bi-analyst`.
