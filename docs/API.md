...
| `GET /api/persons/{id}` | GET | admin | Fetch a single person's complete record for the admin detail/edit view. Returns all `victory_silver.persons` fields (is_current = TRUE) plus nested arrays: `contacts` (`[{ "contact_id": "uuid", "contact_type": "mobile\|facebook\|email", "contact_value": "string", "is_current": true }]`), `occupations` (`[{ "occupation_id": "uuid", "employment_type": "string", "nature_of_work": "string\|null", "company_name": "string\|null", "nature_of_business": "string\|null", "business_name": "string\|null", "is_current": true }]`), and `roles` (`[{ "role_id": "uuid", "role": "string", "assigned_by": "string", "assigned_at": "datetime", "is_active": true }]`). Returns 404 if `person_id` not found or no `is_current = TRUE` record exists. |
...
| `GET /api/event-types` | GET | admin | List all `event_type_catalog` records. Returns: `[{ "event_type_id": "uuid", "type_name": "string", "category": "string", "equipping_step": "string|null", "is_paid": bool }]`. Used to populate the event type dropdown in the event creation form (`POST /api/events`). |
| `GET /api/events` | GET | admin, executive | List all events |
| `POST /api/events` | POST | admin | Create new event instance. Newly created events default to `status = 'closed'` — registration is not publicly accessible until admin explicitly sets `status = 'registration_open'` via `PATCH /api/events/{id}`. |
...
| `POST /api/persons` | POST | admin | Create a new Contact-stage person record directly (admin "Create Profile" tool — accessible from `/admin.html`). Writes to `victory_silver.persons` with `source = 'admin_created'`, `review_status = 'approved'`, `google_uid = NULL`. This is a direct Silver write — no Bronze record is created. Request body: `{ "first_name": "string", "middle_name": "string \| null", "last_name": "string", "suffix": "string \| null", "address": "string", "birthdate": "YYYY-MM-DD", "gender": "M\|F\|other", "civil_status": "single\|married\|widowed\|separated", "email": "string \| null", "contact_number": "string", "facebook_profile": "string \| null" }`. `gender` and `civil_status` are required. Response 201: the created `victory_silver.persons` record with all fields including `person_id`, `source = 'admin_created'`, `review_status = 'approved'`, `google_uid = null`. |
| `POST /api/persons/{id}/contacts` | POST | admin | Create a new `person_contacts` record for a person who has no existing contact of that type. Body: `{ "contact_type": "mobile|facebook|email", "contact_value": "string" }`. Response 201: the created contact record. Used when an admin adds a mobile or Facebook entry that does not yet exist on the person record. |
...
| `POST /api/person-roles` | POST | admin | Assign a role to a person. Body: `{ "person_id": "uuid", "role": "admin|executive" }`. Response 201: the created role record. `assigned_by` resolved from JWT. Writes to `data_change_log`. Note: `vg_leader` role is assigned atomically via `POST /api/persons/{id}/promote-to-leader` — not via this endpoint. |
| `PATCH /api/person-roles/{role_id}` | PATCH | admin | Deactivate a role. Body: `{ "is_active": false }`. Sets `is_active = FALSE` on the specified role record. Writes to `data_change_log`. |
